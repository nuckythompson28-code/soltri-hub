...
for(const mode of [3,4]){
 const result=run(source.replace('#121=3 (3 DEFAULT','#121='+mode+' (3 DEFAULT'));
 const cuts=result.trace.filter(s=>s.seg&&s.seg.tool===2&&s.seg.type===1&&Math.abs(s.seg.x1+65)<1e-8);
 assert.equal(cuts.length,13);cuts.forEach((s,i)=>{assert.ok(Math.abs(s.seg.z1+(i+1)*16.9)<1e-8);assert.ok(s.seg.x0<0);assert.ok(s.seg.x1>s.seg.x0);});
...
console.log('3/4 chamfers, machine 5 directions, UP/DOWN, M00, offsets, IF THEN, missing files/labels and invalid quantity: OK');
