...
function runProgram(text, maxMoves){
  ...
  let moves=0, guard=0, ended=false, endReason='', alarm=null, chamferExtended=null;
  const GUARD=400000;

  function snapKV(){ const o={}; for(const n of KEYVARS) o[n]=vars[n]; return o; }
  function push(o){
    o.state={X:xKnown?X:null,Z:zKnown?Z:null,feed,rpm,sMode,fmode,tool,toolNo,spin,brakeUp,alClamp,mainChuck,zOff,parts,air,chamferExtended};
    o.kv=snapKV();
    trace.push(o);
  }
  ...
  while(!ended){
    ...
    for(const c of Mc){ if(c===3||c===4)spin=machine.cw?(c===machine.cw?'정회전 CW':'역회전 CCW'):'회전'; else if(c===5)spin='정지';
      ...
      if(c===machine.chamferExtend)chamferExtended=true; if(c===machine.chamferRetract)chamferExtended=false;
